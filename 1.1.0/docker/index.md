...
Use `shekel run` as an entrypoint wrapper to enforce LLM cost limits on any agent running inside a Docker container — zero code changes required.
...
The container exits with code 1 if the budget is exceeded, so your orchestration layer (ECS, Kubernetes, Compose) can detect it as a failed task.
...
The `AGENT_BUDGET_USD` env var is equivalent to `--budget N`. This is the preferred pattern for containers because the budget can be set by the operator without rebuilding the image.
...
With `--warn-only`, the container exits 0 even if the budget is exceeded. Use this during development to observe spend without blocking the run.
...
See the [CLI reference](https://arieradle.github.io/shekel/1.1.0/cli/index.md) or use the bundled composite action:
