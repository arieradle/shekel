...
Supported units: `s`, `sec`, `min`, `hr`, `h`. Calendar units (`day`, `week`, `month`) are intentionally **not supported** — rolling windows only.
...
Redis backend — available now

`RedisBackend` and `AsyncRedisBackend` ship with shekel v1.0.2 and implement this protocol with atomic Lua-script enforcement (one round-trip), a circuit breaker, and fail-closed/open modes. See [Distributed Budgets](https://arieradle.github.io/shekel/1.1.0/usage/distributed-budgets/index.md) or `from shekel.backends.redis import RedisBackend`.
