...
1. `Budget.__enter__`: set `_active_budget` to this budget, call `apply_patches()` (refcount 1 → install all adapters).
1. User calls `client.chat.completions.create(...)` (or another patched method).
1. Patched wrapper runs: resolve provider, apply fallback model if active, call original; get response or stream.
1. If streaming: adapter's `wrap_stream` consumes the stream and yields chunks; at end, token counts are returned and used for cost.
1. Token extraction (adapter) → `calculate_cost` (pricing) → `budget._record_*` (update spent, check limits).
1. If over limit: raise `BudgetExceededError` (or `ToolBudgetExceededError` for tools). Optionally emit observability events.
1. On `with` exit: `remove_patches()` (refcount 0 → restore originals), clear `_active_budget`, emit exit events.
...
1. CLI parses flags and optional `--budget-file` / env into budget kwargs.
1. Run layer (e.g. `_run_utils`) creates a `Budget` with those kwargs and enters it.
1. User script is executed (same process or subprocess depending on implementation); any patched SDK calls are tracked and enforced.
1. On script end, budget context exits; CLI reports spend and exit code (e.g. 1 if budget exceeded).
