...
1. Before each dispatch, it counts how many calls to that tool fall within the rolling window
1. If the count reaches `loop_guard_max_calls`, `AgentLoopError` is raised **before the tool executes**
1. Old timestamps outside the window are evicted lazily on each check
...
- [Spend Velocity](https://arieradle.github.io/shekel/1.2.0/usage/spend-velocity/index.md) — burn-rate circuit breaker
- [Tool Budgets](https://arieradle.github.io/shekel/1.2.0/usage/tool-budgets/index.md) — cap total tool calls and charge per tool
