...
2. The exception contains detailed information
3. No further API calls are allowed in that context

!!! danger "Hard Caps Stop Execution"
    When `max_usd` is exceeded, shekel raises an exception immediately. The current API call completes, but no further calls are made. Use [fallback models](fallback-models.md) if you want to continue with a cheaper model instead.
...
!!! tip "Automatic Fallback"
    Instead of manually catching and retrying, consider using [fallback models](fallback-models.md) for automatic model switching.
